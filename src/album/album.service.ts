...
export class AlbumService {
  ...
  async createAlbum(CreateAlbumDto: CreateAlbumDto) {
    if (
      Object.keys(CreateAlbumDto).length >= 4 ||
    ...
    try {
      ...
      throw new HttpException(
        `May be artist with id === ${CreateAlbumDto.artistId} doesn't exist`,
        404,
      );
  ...
  async updateAlbumById(UpdateDataAlbumDto: UpdateDataAlbumDto, id: string) {
    ...
    for (const key of keys) {
      if (
        ...
        typeof UpdateDataAlbumDto.name !== 'string' ||
        typeof UpdateDataAlbumDto.year !== 'number' ||
        (typeof UpdateDataAlbumDto.artistId !== 'string' &&
          UpdateDataAlbumDto.artistId !== null)
      ) {
        throw new HttpException(
